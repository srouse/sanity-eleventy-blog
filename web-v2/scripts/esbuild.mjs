...
export default async function esbuild (
  ...
  const entryFiles = await glob(path.resolve('./src/**/!(*.test).ts'));// assume the root

  const dist = isNode ? './dist-node' : './dist';
  const plugins = buildPlugins(dist);
...
function buildPlugins(dist) {// incremental = false) {
  const plugins = [
    /* sassPlugin({
      type: "lit-css",
